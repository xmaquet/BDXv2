...
def parse_nmcli_wifi_list(text: str) -> list[dict[str, Any]]:
    ...
    for raw in text.splitlines():
        ...
        if len(parts) < 5:
            continue
        in_use, ssid, signal_s, security, freq_s = parts[0], parts[1], parts[2], parts[3], parts[4]
        ssid = ssid.strip()
        if not ssid or ssid == "--":
            continue
...
class RobotWifi:
    ...
    def _status(self) -> None:
        try:
            ...
            self._push(_state(None, "failed", None, None, _SUDOERS_HINT))
            return
        except Exception as e:
            self._push(_state(None, "failed", None, None, str(e)[:180]))
            return
    ...
    def _join(self, obj: dict[str, Any]) -> dict[str, Any]:
        ...
        self._push(_state(ssid, "connecting", None, None, "Association…"))
        threading.Thread(target=self._join_run, args=(ssid, psk), daemon=True).start()
        return ack

    def _join_run(self, ssid: str, psk: str) -> None:
        try:
            self._run("join", ssid, psk)
        except FileNotFoundError:
            self._push(_state(ssid, "failed", None, None, _SUDOERS_HINT))
            self._ack("join", False, _SUDOERS_HINT)
        except Exception as e:
            msg = str(e)[:180]
            self._push(_state(ssid, "failed", None, None, msg))
